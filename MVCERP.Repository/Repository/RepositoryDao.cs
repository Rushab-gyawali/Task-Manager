      ...
      public MVCERP.Shared.Common.DbResponse ParseDbResponse(System.Data.DataTable dt)
        ...
        public System.Collections.Generic.Dictionary<string, string> ParseDictionary(string sql)
        {
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                ...
                //{
                dictionary.Add(dt.Rows[i][1].ToString(), dt.Rows[i][0].ToString());
                //}
            }
            return dictionary;
        }
        ...
        public ReportResultCommon ParseReportResult(string sql)
        {
            var ds = ExecuteDataset(sql);
            return ParseReportResult(ds, sql);
        }
        #region ParseReportResult
        public ReportResultCommon ParseReportResult(DataSet ds, string sql)
        {
            var res = new ReportResultCommon();
            ...
            if (tableCount > 1)
            {
                ...
                var dbresult = ParseDbResponse(ds.Tables[pos]);
        ...
        public ReportResultCommon ParseReportResult(DataSet ds)
        {
